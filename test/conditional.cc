...
TEST(ConditionalTest, Then) {
    auto then = []() {
        return Eventual<std::string>().start(
            [](auto& k) { succeed(k, "then"); });
    };

    auto els3 = []() {
        return Eventual<std::string>().start(
            [](auto& k) { succeed(k, "else"); });
    };

    auto c = [&]() {
        return Just(1) | [](int i) { return i + 1; } |
               Conditional([](auto&& i) { return i > 1; },
                           [&](auto&&) { return then(); },
                           [&](auto&&) { return els3(); });
    };
...
TEST(ConditionalTest, Else) {
    ...
    auto c = [&]() {
        return Just(0) | [](int i) { return i + 1; } |
               Conditional([](auto&& i) { return i > 1; },
                           [&](auto&&) { return then(); },
                           [&](auto&&) { return els3(); });
    };
