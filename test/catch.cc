...
TEST(CatchTest, RaisedRuntimeError) {
  auto e = []() {
    return Just(1) >> Raise(RuntimeError("message")) >>
           Catch().raised<RuntimeError>([](RuntimeError &&error) {
  ...
  static_assert(eventuals::tuple_types_unordered_equals_v<
                decltype(e())::ErrorsFrom<void, std::tuple<>>, std::tuple<>>);
...
TEST(CatchTest, ChildException) {
  struct MyError : public Error {
    const char *what() const noexcept override { return "child exception"; }
  };

  auto e = []() {
    return Just(1) >> Raise(MyError{}) >>
           Catch().raised<TypeErasedError>([](TypeErasedError &&error) {
...
TEST(CatchTest, All) {
  auto e = []() {
    return Just(500) >> Raise(RuntimeError("10")) >>
           Catch().all([](std::variant<RuntimeError> &&error) {
...
TEST(CatchTest, AllRaisedOneException) {
  ...
  auto e = []() {
    return Just(500) >> Raise(RuntimeError("runtime_error")) >>
           Raise(MyError{}) >>
           Catch()
               ...
               .all([](std::variant<RuntimeError> &&error) {
                 EXPECT_STREQ(std::get<RuntimeError>(error).what(),
                              "runtime_error");
                 return 100;
               }) >>
           Then([](int value) { return value; });
...
TEST(CatchTest, UnexpectedRaise) {
  ...
  auto e = [&]() {
    return f() >> Catch().raised<MyError>([](MyError &&error) {
      EXPECT_EQ("child exception", error.what());
      return 100;
    });
  };
...
TEST(CatchTest, UnexpectedAll) {
  ...
  auto e = [&]() {
    return f() >> Catch().all([](std::variant<MyError> &&error) {
...
TEST(CatchTest, NoExactHandler) {
  ...
  auto e = []() {
    return Just(1) >> Raise(std::string("error")) >>
           Catch().raised<MyError>([](MyError &&error) {
             FAIL() << "Encountered unexpected matched raised";
           });
  };
...
TEST(CatchTest, VoidPropagate) {
  auto e = []() {
    return Just("some string") >> Then([](const char *i) { return; }) >>
           Raise("error") >>
           Catch().raised<TypeErasedError>([](TypeErasedError &&error) {
             EXPECT_STREQ(error.what(), "error");
             // MUST RETURN VOID HERE!
           }) >>
           Then([](/* MUST TAKE VOID HERE! */) { return 100; });
  };
...
TEST(CatchTest, RaiseFromCatch) {
  auto all = []() {
    return Just(1) >> Raise("10") >>
           Catch().all([](auto &&error) { return Just(10) >> Raise("1"); });
  };

  auto raised = []() {
    return Just(1) >> Raise("10") >>
           Catch().raised<RuntimeError>(
               [](auto &&error) { return Just(10) >> Raise("1"); });
  };

  // Be careful there!
  static_assert(eventuals::tuple_types_unordered_equals_v<
                decltype(all())::ErrorsFrom<void, std::tuple<>>, std::tuple<>>);
