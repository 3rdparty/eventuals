    ...
    auto s = [&]() {
        return Stream<int>()
               ...
               (Loop<int>()
                    ...
                    .ended([](auto& sum, auto& k) { succeed(k, sum); })
...
TEST(StreamTest, Done) {
    ...
    auto s = [&]() {
        return Stream<int>()
                   .context(0)
                   .next([](auto& value, auto& k) { emit(k, value); })
                   .done([](auto&, auto& k) { ended(k); }) |
               (Loop<int>()
                    .context(0)
                    ...
                    .ended([](auto& count, auto& k) { succeed(k, count); })
                    .fail([&](auto&, auto&, auto&&) { fail.Call(); })
                    .stop([&](auto&, auto&) { stop.Call(); }));
    };
...
TEST(StreamTest, Fail) {
    ...
    EXPECT_CALL(ended, Call()).Times(0);

    auto s = [&]() {
        return Stream<int>()
                   .context("error")
                   .next(
                       [](auto& error, auto& k) { eventuals::fail(k, error); })
                   .done([&](auto&, auto&) { done.Call(); }) |
               (Loop<int>()
                    .context(0)
                    .body([](auto&, auto& stream, auto&&) { next(stream); })
                    .ended([&](auto&, auto&) { ended.Call(); })
...
TEST(StreamTest, InterruptStream) {
    ...
    auto              s         = [&]() {
        return Stream<int>()
               ...
               (Loop<int>()
                    .body([&](auto& k, auto&&) {
                        auto thread = std::thread([&]() mutable {
...
TEST(StreamTest, InfiniteLoop) {
    auto s = []() {
        return Stream<int>().context(5).next([](auto& count, auto& k) {
            if (count > 0) {
                emit(k, count--);
            } else {
                ended(k);
...
TEST(StreamTest, MapReduceEventual) {
    auto s = [&]() {
        return Stream<int>()
                   ...
                   .done([](auto&, auto& k) { ended(k); }) |
               Map(Eventual<int>().start(
                   [](auto& k, auto&& i) { succeed(k, i + 1); })) |
               Reduce(
                   /* sum = */ 0, [](auto& sum) {
                       return [&](auto&& value) {
                           sum += value;
