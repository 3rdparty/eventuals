...
TEST_F(TCPIPV6Test, SocketSendReceiveSuccess) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  ...
  auto e_receive_from_socket = [&]() {
    return accepted.Receive(
        buffer,
        sizeof(buffer),
        TCPIPV6Test::kTestDataSize);
  };
  ...
  auto e_send_to_socket = [&]() {
    return accepted.Send(TCPIPV6Test::kTestData, TCPIPV6Test::kTestDataSize);
  };
...
TEST_F(TCPIPV6Test, SocketSendReceiveClosedFail) {
  Socket socket(Protocol::IPV6);
...
TEST_F(TCPIPV6Test, SocketSendReceiveNotConnectedFail) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  Socket socket(Protocol::IPV6);
...
TEST_F(TCPIPV6Test, SocketSendReceiveInterruptBeforeStart) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  Acceptor acceptor(Protocol::IPV6);
...
// calling interrupt.Trigger() before and after k.Start()
// since Send and Receive operations are asynchronous.
TEST_F(TCPIPV6Test, SocketSendReceiveInterruptAfterStart) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  Acceptor acceptor(Protocol::IPV6);
