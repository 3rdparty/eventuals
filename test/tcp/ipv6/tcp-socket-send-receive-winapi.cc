...
TEST_F(TCPIPV6Test, SocketEchoServerWinApi) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  Socket socket;
  ...
  auto e_setup = [&]() {
    ...
    address.sin6_port = htons(TCPIPV6Test::kAnyPort);
    inet_pton(AF_INET6, TCPIPV6Test::kLocalHostIPV6, &address.sin6_addr);

    socket_winapi = ::socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
  ...
  auto e_connect = [&]() {
    return socket.Connect(TCPIPV6Test::kLocalHostIPV6, socket_port);
  };
  ...
  char buffer[TCPIPV6Test::kTestDataSize];
  memset(buffer, 0, sizeof(buffer));

  auto e_send_to_winapi = [&]() {
    return socket.Send(TCPIPV6Test::kTestData, TCPIPV6Test::kTestDataSize);
  };
  ...
  auto e_receive_from_socket = [&]() {
    return Eventual<void>()
        .start([&](auto& k) {
          winapi_thread = std::thread([&]() {
            size_t bytes_to_read = TCPIPV6Test::kTestDataSize;
            size_t bytes_read = 0;
  ...
  EXPECT_EQ(
      0,
      memcmp(TCPIPV6Test::kTestData, buffer, TCPIPV6Test::kTestDataSize));
  ...
  auto e_send_to_socket = [&]() {
    return Eventual<void>()
        .start([&](auto& k) {
          winapi_thread = std::thread([&]() {
            size_t bytes_to_write = TCPIPV6Test::kTestDataSize;
            size_t bytes_written = 0;

            while (bytes_written < bytes_to_write) {
              const char* data_to_send_ptr = TCPIPV6Test::kTestData
                  + bytes_written;
  ...
  auto e_receive_from_winapi = [&]() {
    return socket.Receive(buffer, sizeof(buffer), TCPIPV6Test::kTestDataSize);
  };
