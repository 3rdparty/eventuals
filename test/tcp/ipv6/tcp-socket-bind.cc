...
TEST_F(TCPIPV6Test, SocketBindBadIPFail) {
  ...
  auto e = socket.Open()
      >> socket.Bind("::H", TCPIPV6Test::kAnyPort);
...
TEST_F(TCPIPV6Test, SocketBindClosedFail) {
  Socket socket(Protocol::IPV6);

  eventuals::Interrupt interrupt;

  auto e = [&]() {
    return socket.Bind(TCPIPV6Test::kAnyIPV6, TCPIPV6Test::kAnyPort);
  };
...
TEST_F(TCPIPV6Test, SocketBindWhileConnectedFail) {
  // ---------------------------------------------------------------------
  // Setup section.
  // ---------------------------------------------------------------------
  ...
  auto e = [&]() {
    return socket.Bind(TCPIPV6Test::kLocalHostIPV6, TCPIPV6Test::kAnyPort);
  };
...
TEST_F(TCPIPV6Test, SocketBindInterrupt) {
  // ---------------------------------------------------------------------
  // Main section.
  // ---------------------------------------------------------------------
  Socket socket(Protocol::IPV6);
