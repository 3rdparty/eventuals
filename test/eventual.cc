...
TEST(EventualTest, Succeed) {
    ...
    EXPECT_CALL(fail, Call()).Times(0);

    EXPECT_CALL(stop, Call()).Times(0);

    auto e = [&]() {
        return Eventual<int>().context(5).start([](auto& context, auto& k) {
            auto thread =
                std::thread([&context, &k]() mutable { succeed(k, context); });
            thread.detach();
        }) | [](int i) { return i + 2; } |
               (Eventual<int>()
                    .context(9)
                    .start([](auto& context, auto& k, auto&& value) {
                        auto thread =
                            std::thread([value, &context, &k]() mutable {
                                succeed(k, context - value);
                            });
                        thread.detach();
                    })
                    .fail([&](auto&, auto&, auto&&) { fail.Call(); })
                    .stop([&](auto&, auto&) { stop.Call(); }));
    };
...
TEST(EventualTest, Fail) {
    ...
    EXPECT_CALL(start, Call()).Times(0);

    EXPECT_CALL(stop, Call()).Times(0);

    auto e = [&]() {
        return Eventual<int>().context("error").start([](auto& error,
                                                         auto& k) {
            auto thread =
                std::thread([&error, &k]() mutable { fail(k, error); });
            thread.detach();
        }) | [](int i) { return i + 2; } |
               (Eventual<int>()
                    .start([&](auto& k, auto&& value) { start.Call(); })
                    .stop([&](auto&) { stop.Call(); }));
    };
...
TEST(EventualTest, Interrupt) {
    ...
    auto e = [&]() {
        return Eventual<int>()
                   .context(5)
                   .start([&](auto&, auto& k) { start.Call(); })
                   .interrupt([](auto&, auto& k) { stop(k); }) |
               [](int i) { return i + 2; } |
               (Eventual<int>()
                    .start([&](auto&, auto&&) { start.Call(); })
                    .fail([&](auto&, auto&&) { fail.Call(); })
                    .stop([](auto& k) { stop(k); }));
    };
...
TEST(EventualTest, Reuse) {
    auto operation = [](int i, auto&& promise) {
        return (Eventual<int>().context(i).start([](auto& context, auto& k) {
                   auto thread = std::thread(
                       [&context, &k]() mutable { succeed(k, context); });
                   thread.detach();
               })) |
               [](int i) { return i + 2; } |
               (Eventual<int>().context(9).start([](auto& context, auto& k,
                                                    auto&& value) {
                   auto thread = std::thread([value, &context, &k]() mutable {
                       succeed(k, context - value);
                   });
                   thread.detach();
               })) |
...
TEST(EventualTest, Just) {
    auto e = []() { return Just(42); };

    EXPECT_EQ(42, *e());
}

TEST(EventualTest, Raise) {
    auto e = []() { return Raise("error"); };
...
TEST(EventualTest, Lambda) {
    auto e = []() {
        return Just(42) | Lambda([](auto i) { return i + 1; }) |
               [](auto j) { return j - 1; };
    };
