...
TEST(RepeatTest, Succeed) {
    ...
    auto r = [&]() {
        return Eventual<deque<string>>().start([](auto& k) {
            auto thread = std::thread([&k]() mutable {
                succeed(k, deque<string>{ "hello", "world" });
            });
            thread.detach();
        }) | Repeat() |
               Until([](auto& strings) { return strings.empty(); }) |
               Map([&](auto&& strings) {
...
TEST(RepeatTest, Fail) {
    auto e = [](auto) {
        return Eventual<string>().start([](auto& k) { fail(k, "error"); });
    };
...
TEST(RepeatTest, Eventual) {
    auto r = []() {
        return Repeat(Eventual<int>().start([](auto& k) { succeed(k, 1); })) |
               (Loop<int>()
                    .context(0)
...
TEST(RepeatTest, Map) {
    auto r = []() {
        return Repeat() |
               Map(Eventual<int>().start([](auto& k) { succeed(k, 1); })) |
               (Loop<int>()
                    .context(0)
                    .body([](auto&& count, auto& repeated, auto&& value) {
                        count += value;
...
TEST(RepeatTest, MapAcquire) {
    ...
    auto r = [&]() {
        return Repeat(Eventual<int>().start([](auto& k) { succeed(k, 1); })) |
               Map(Acquire(&lock) |
                   (Wait<int>(&lock).condition(
                       [](auto& k, auto&& i) { succeed(k, i); })) |
                   Lambda([](auto&& i) { return i; }) | Release(&lock)) |
               (Loop<int>()
                    .context(0)
                    .body([](auto&& count, auto& repeated, auto&& value) {
                        count += value;
