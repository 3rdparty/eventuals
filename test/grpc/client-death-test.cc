...
TEST_F(EventualsGrpcTest, ClientDeathTest) {
  ...
  // TODO(alexmc, benh): Is "cancelled" the correct name here? The server
  // doesn't cancel after a single aborted RPC, does it?
  ...
  std::cout << "Running client with command: " << command << std::endl;
  ...
  EXPECT_EQ(17, WEXITSTATUS(result));
