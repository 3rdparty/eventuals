...
struct Map {
    ...
    Map(K_ k, E_ e) : k_(std::move(k)), e_(std::move(e)) {}
        ...
        if (!adaptor_) {
            ...
            adaptor_.emplace(
                ...
                Adaptor<K_, typename E_::Value>(
                    k_, [&k](auto& k_, auto&&... values) {
                        eventuals::body(
                            k_, k, std::forward<decltype(values)>(values)...);
                    }));
...
template<typename K, typename E>
struct IsTransform<detail::Map<K, E>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename E>
struct IsContinuation<detail::Map<K, E>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename E>
struct HasLoop<detail::Map<K, E>> : HasLoop<K> {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename E>
struct HasTerminal<detail::Map<K, E>> : HasTerminal<K> {};
...
struct Compose<detail::Map<K, E>> {
    ...
    static auto compose(detail::Map<K, E> map) {
        ...
        auto k =
            eventuals::compose<typename decltype(e)::Value>(std::move(map.k_));
        return detail::Map<decltype(k), decltype(e)>(std::move(k),
                                                     std::move(e));
    }
};
...
template<typename E, std::enable_if_t<IsContinuation<E>::value, int> = 0>
auto Map(E e) {
