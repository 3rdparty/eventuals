...
void emit(K& k, Args&&... args) {
    static_assert(!std::is_same_v<K, Undefined>,
...
template<typename K_, typename Context_, typename Start_, typename Next_,
         typename Done_, typename Fail_, typename Stop_, typename Interrupt_,
         typename Value_, typename... Errors_>
struct Stream {
    ...
    template<typename Value, typename... Errors, typename K, typename Context,
             typename Start, typename Next, typename Done, typename Fail,
             typename Stop, typename Interrupt>
    static auto create(K k, Context context, Start start, Next next, Done done,
                       Fail fail, Stop stop, Interrupt interrupt) {
        return Stream<K, Context, Start, Next, Done, Fail, Stop, Interrupt,
                      Value, Errors...>{
            std::move(k),    std::move(context),  std::move(start),
            std::move(next), std::move(done),     std::move(fail),
            std::move(stop), std::move(interrupt)
        };
    }

    template<typename L, std::enable_if_t<IsLoop<L>::value, int> = 0>
    auto k(L l) && {
        static_assert(!HasLoop<K_>::value, "Redundant 'Loop'");
        ...
        auto k = create<Value_, Errors_...>(
            ...
            std::move(context_), std::move(start_), std::move(next_),
            std::move(done_), std::move(fail_), std::move(stop_),
            std::move(interrupt_));

        return Eventual<Undefined, Undefined, Undefined, Undefined, Undefined,
                        Undefined, Undefined>::
            create<typename L::Value>(
                std::move(k), Undefined(),
                ...
                // current 'Undefined()').
                [](auto& k) { eventuals::start(k); }, Undefined(),
                [](auto& k) { eventuals::stop(k); }, Undefined());
    }

    template<typename K,
             std::enable_if_t<!IsLoop<K>::value && IsContinuation<K>::value,
                              int> = 0>
    auto k(K k) && {
        static_assert(!IsTransform<K>::value || !HasLoop<K_>::value,
                      "Can't add 'Transform' *after* 'Loop'");

        static_assert(!IsTerminal<K>::value || HasLoop<K_>::value,
    ...
    template<typename F,
             std::enable_if_t<!IsLoop<F>::value && !IsContinuation<F>::value,
                              int> = 0>
    auto k(F f) && {
    ...
    auto context(Context context) && {
        ...
        return create<Value_, Errors_...>(
            ...
            std::move(next_), std::move(done_), std::move(fail_),
    ...
    auto next(Next next) && {
        ...
        return create<Value_, Errors_...>(
            ...
            std::move(next), std::move(done_), std::move(fail_),
            std::move(stop_), std::move(interrupt_));
    }
    ...
    auto done(Done done) && {
        ...
        return create<Value_, Errors_...>(
            std::move(k_), std::move(context_), std::move(start_),
            std::move(next_), std::move(done), std::move(fail_),
            std::move(stop_), std::move(interrupt_));
    }
    ...
    auto fail(Fail fail) && {
        ...
        return create<Value_, Errors_...>(
            std::move(k_), std::move(context_), std::move(start_),
            std::move(next_), std::move(done_), std::move(fail),
            std::move(stop_), std::move(interrupt_));
    }
    ...
    auto stop(Stop stop) && {
        ...
        return create<Value_, Errors_...>(
            std::move(k_), std::move(context_), std::move(start_),
            std::move(next_), std::move(done_), std::move(fail_),
            std::move(stop), std::move(interrupt_));
    }
    ...
    auto interrupt(Interrupt interrupt) && {
        ...
        return create<Value_, Errors_...>(
            std::move(k_), std::move(context_), std::move(start_),
            std::move(next_), std::move(done_), std::move(fail_),
            std::move(stop_), std::move(interrupt));
    }
    ...
    void Next() {
        static_assert(!IsUndefined<Next_>::value,
    ...
    void Done() {
        static_assert(!IsUndefined<Done_>::value,
...
template<typename K, typename Context, typename Start, typename Next,
         typename Done, typename Fail, typename Stop, typename Interrupt,
         typename Value, typename... Errors>
struct IsStream<detail::Stream<K, Context, Start, Next, Done, Fail, Stop,
...
template<typename K, typename Context, typename Start, typename Next,
         typename Done, typename Fail, typename Stop, typename Interrupt,
         typename Value, typename... Errors>
struct IsContinuation<detail::Stream<K, Context, Start, Next, Done, Fail, Stop,
                                     Interrupt, Value, Errors...>> :
    std::true_type {};
...
template<typename K, typename Context, typename Start, typename Next,
         typename Done, typename Fail, typename Stop, typename Interrupt,
         typename Value, typename... Errors>
struct HasLoop<detail::Stream<K, Context, Start, Next, Done, Fail, Stop,
                              Interrupt, Value, Errors...>> : HasLoop<K> {};

template<typename K, typename Context, typename Start, typename Next,
         typename Done, typename Fail, typename Stop, typename Interrupt,
         typename Value, typename... Errors>
struct HasTerminal<detail::Stream<K, Context, Start, Next, Done, Fail, Stop,
                                  Interrupt, Value, Errors...>> :
    HasTerminal<K> {};

template<typename S, typename K>
struct HasTerminal<detail::StreamK<S, K>> : HasTerminal<K> {};

template<typename Value, typename... Errors>
auto Stream() {
    return detail::Stream<Undefined, Undefined, Undefined, Undefined,
