...
struct IsContinuation<detail::Lambda<K, F, Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename F, typename Arg>
struct HasTerminal<detail::Lambda<K, F, Arg>> : HasTerminal<K> {};
...
struct Compose<detail::Lambda<K, F, Arg_>> {
    ...
    static auto compose(detail::Lambda<K, F, Arg_> lambda) {
        ...
        return detail::Lambda<decltype(k), F, Value>(std::move(k),
                                                     std::move(lambda.f_));
    }
};
