...
public:
    ...
    void Submit(Callback<> callback, void* context,
        ...
        if (!defer && StaticThreadPool::member &&
            ...
            while (!head->compare_exchange_weak(waiter->next, waiter,
...
struct StaticThreadPoolSchedule : public StaticThreadPool::Waiter {
    ...
    using Value =
        typename ValueFrom<K_,
                           typename ValuePossiblyUndefined<E_>::Value>::type;

    StaticThreadPoolSchedule(K_ k, StaticThreadPool* pool,
    ...
    static auto create(K k, StaticThreadPool* pool,
                       StaticThreadPool::Requirements* requirements, F f) {
        return StaticThreadPoolSchedule<K, F, Arg>(std::move(k), pool,
                                                   requirements, std::move(f));
    }
    ...
    using Adaptor_                 = typename EKPossiblyUndefined<
        E_, typename EKPossiblyUndefined<
                decltype(scheduler_->Reschedule(context_)),
                Adaptor<K_, typename ValuePossiblyUndefined<E_>::Value>>::
                type>::type;
...
struct IsContinuation<detail::StaticThreadPoolSchedule<K, F, Arg>> :
    std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename F, typename Arg>
struct HasTerminal<detail::StaticThreadPoolSchedule<K, F, Arg>> :
    HasTerminal<K> {};
...
struct Compose<detail::StaticThreadPoolSchedule<K, F, Arg_>> {
    ...
    static auto compose(
        detail::StaticThreadPoolSchedule<K, F, Arg_> schedule) {
        if constexpr (!IsUndefined<Arg>::value) {
            ...
            return detail::StaticThreadPoolSchedule<decltype(k), F, Arg>(
                std::move(k), schedule.pool(), schedule.requirements(),
...
StaticThreadPool::StaticThreadPool() :
                ...
                if (waiter != nullptr) {
                    if (waiter->next == nullptr) {
                        if (!head.compare_exchange_weak(
                                waiter, nullptr, std::memory_order_release,
                                std::memory_order_relaxed)) {
                            goto load;   // Try again.
                        }
                    } else {
    ...
    for (size_t core = 0; core < concurrency; core++) { ready_[core].Wait(); }
}
...
auto StaticThreadPool::Schedule(Requirements* requirements, F f) {
    return detail::StaticThreadPoolSchedule<Undefined, F, Undefined>(
        Undefined(), this, requirements, std::move(f));
}
