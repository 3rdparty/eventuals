...
struct Reduce {
    ...
    void Body(K& k, Args&&... args) {
        ...
        static_assert(IsUndefined<Value>::value || std::is_same_v<bool, Value>,
                      "Expecting 'Reduce' to eventually return a bool");

        adaptor_.emplace(std::move(e) |
                         Adaptor<K_, bool>(k_, [&k](auto& k_, bool next) {
                             if (next) {
                                 eventuals::next(k);
                             } else {
                                 eventuals::done(k);
    ...
    void Ended() { eventuals::succeed(k_, std::move(t_)); }
    ...
    using E_      = std::conditional_t<IsContinuation<Result_>::value, Result_,
                                  Lambda<Undefined, Result_, Arg_>>;

    using Adaptor_ = typename EKPossiblyUndefined<E_, Adaptor<K_, bool>>::type;
...
struct IsContinuation<detail::Reduce<K, T, F, Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename T, typename F, typename Arg>
struct HasTerminal<detail::Reduce<K, T, F, Arg>> : HasTerminal<K> {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename T, typename F, typename Arg>
struct IsLoop<detail::Reduce<K, T, F, Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename T, typename F, typename Arg>
struct HasLoop<detail::Reduce<K, T, F, Arg>> : std::true_type {};
...
struct Compose<detail::Reduce<K, T, F, Arg_>> {
    ...
    static auto compose(detail::Reduce<K, T, F, Arg_> reduce) {
        ...
        static_assert(
            IsContinuation<Result>::value || std::is_invocable_v<Result, Arg>,
        ...
        return detail::Reduce<K, T, F, Arg>(
            std::move(reduce.k_), std::move(reduce.t_), std::move(reduce.f_));
    }
};
...
auto Reduce(T t, F f) {
    ...
    return detail::Reduce<Undefined, T, F, Undefined>(
        Undefined(), std::move(t), std::move(f));
}
