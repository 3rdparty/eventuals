...
struct Adaptor {
    ...
    Adaptor(K_& k, Ts_... ts, Start start) :
        k_(k),
        ts_(std::forward<Ts_>(ts)...),
        start_(std::move(start)) {}

    Adaptor(Adaptor&& that) :
        k_(that.k_),
        ts_(std::move(that.ts_)),
        start_(std::move(that.start_)) {}

    K_&                k_;
    std::tuple<Ts_...> ts_;
    std::conditional_t<!IsUndefined<Arg_>::value,
                       Callback<K_&, Ts_..., Arg_&&>, Callback<K_&, Ts_...>>
        start_;

    template<typename... Args>
    void Start(Args&&... args) {
        static_assert((IsUndefined<Arg_>::value && sizeof...(args) == 0) ||
                          sizeof...(args) == 1,
        ...
        std::apply(
            [&](auto&&... ts) {
                assert(start_);
                start_(k_, std::forward<decltype(ts)>(ts)...,
...
struct IsContinuation<detail::Adaptor<K, Arg, Ts...>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Arg, typename... Ts>
struct HasTerminal<detail::Adaptor<K, Arg, Ts...>> : HasTerminal<K> {};
