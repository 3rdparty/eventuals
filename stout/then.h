...
struct Then {
    ...
    void Start(Args&&... args) {
        adaptor_.emplace(
            f_(std::forward<Args>(args)...)
                .k(Adaptor<K_, typename E_::Value>(
    ...
    using Adaptor_        = typename EKPossiblyUndefined<
        E_, Adaptor<K_, typename ValuePossiblyUndefined<E_>::Value>>::type;
...
struct IsContinuation<detail::Then<K, F, Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename F, typename Arg>
struct HasTerminal<detail::Then<K, F, Arg>> : HasTerminal<K> {};
...
struct Compose<detail::Then<K, F, Arg_>> {
    ...
    static auto compose(detail::Then<K, F, Arg_> then) {
        if constexpr (!IsUndefined<Arg>::value) {
            ...
            return detail::Then<decltype(k), F, Arg>(std::move(k),
                                                     std::move(then.f_));
        } else {
