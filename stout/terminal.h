...
template<typename Context_, typename Start_, typename Fail_, typename Stop_,
         typename Interrupt_>
struct Terminal {
    ...
    template<typename Context, typename Start, typename Fail, typename Stop,
             typename Interrupt>
    static auto create(Context context, Start start, Fail fail, Stop stop,
                       Interrupt interrupt) {
        return Terminal<Context, Start, Fail, Stop, Interrupt>{
            std::move(context), std::move(start), std::move(fail),
    ...
    auto context(Context context) && {
        ...
        return create(std::move(context), std::move(start_), std::move(fail_),
    ...
    auto start(Start start) && {
        ...
        return create(std::move(context_), std::move(start), std::move(fail_),
                      std::move(stop_), std::move(interrupt_));
    }
    ...
    auto fail(Fail fail) && {
        ...
        return create(std::move(context_), std::move(start_), std::move(fail),
                      std::move(stop_), std::move(interrupt_));
    }
    ...
    auto stop(Stop stop) && {
        ...
        return create(std::move(context_), std::move(start_), std::move(fail_),
    ...
    template<typename... Args>
    void Fail(Args&&... args) {
        static_assert(!IsUndefined<Fail_>::value,
    ...
    void Stop() {
        static_assert(!IsUndefined<Stop_>::value,
...
template<typename Context, typename Start, typename Fail, typename Stop,
         typename Interrupt>
struct IsTerminal<detail::Terminal<Context, Start, Fail, Stop, Interrupt>> :
    std::true_type {};
...
template<typename Context, typename Start, typename Fail, typename Stop,
         typename Interrupt>
struct HasTerminal<detail::Terminal<Context, Start, Fail, Stop, Interrupt>> :
    std::true_type {};

template<typename Context, typename Start, typename Fail, typename Stop,
         typename Interrupt, typename Value>
struct ValueFrom<detail::Terminal<Context, Start, Fail, Stop, Interrupt>,
                 Value> {
    using type = Value;
};
