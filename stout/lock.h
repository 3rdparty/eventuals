...
public:
    ...
    bool AcquireFast(Waiter* waiter) {
        ...
        while (waiter->next == nullptr) {
            if (head_.compare_exchange_weak(waiter->next, waiter,
    ...
    void AcquireSlow(Waiter* waiter) {
        ...
        while (!head_.compare_exchange_weak(waiter->next, waiter,
                                            std::memory_order_release,
                                            std::memory_order_relaxed))
            ;
        ...
        // continuation.
        if (waiter->next == nullptr) { waiter->f(); }
    }
    ...
    void Release() {
        ...
        if (waiter->next == nullptr) {
            if (!head_.compare_exchange_weak(waiter, nullptr,
                                             std::memory_order_release,
                                             std::memory_order_relaxed)) {
                return Release();   // Try again.
            }
        } else {
            while (waiter->next->next != nullptr) { waiter = waiter->next; }
            waiter->next = nullptr;
            waiter->f();
        }
    }
...
struct Acquire {
    ...
    void Stop() {
        if (lock_->AcquireFast(&waiter_)) {
            ...
            waiter_.f  = [this]() mutable {
                scheduler_->Submit([this]() mutable { eventuals::stop(k_); },
                                   context_);
            };
...
struct Release {
    ...
    Release(Release&& that) : k_(std::move(that.k_)), lock_(that.lock_) {}
...
struct Wait {
    ...
    template<typename Arg, typename Value, typename K, typename Context,
             typename Condition>
    static auto create(K k, Context context, Condition condition, Lock* lock) {
        return Wait<K, Context, Condition, Arg, Value>(
            std::move(k), std::move(context), std::move(condition), lock);
    }
    ...
    auto k(K k) && {
        return create<Arg_, Value_>(
            ...
            std::move(context_), std::move(condition_), lock_);
    }
    ...
    auto context(Context context) && {
        ...
        return create<Arg_, Value_>(std::move(k_), std::move(context),
                                    std::move(condition_), lock_);
    }

    template<typename Condition>
    auto condition(Condition condition) && {
        static_assert(IsUndefined<Condition_>::value, "Duplicate 'condition'");
        return create<Arg_, Value_>(std::move(k_), std::move(context_),
                                    std::move(condition), lock_);
    }

    template<typename... Args>
    void Start(Args&&... args) {
        static_assert(!IsUndefined<Condition_>::value,
...
struct IsContinuation<detail::Acquire<K, Value>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Value>
struct HasTerminal<detail::Acquire<K, Value>> : HasTerminal<K> {};
...
struct Compose<detail::Acquire<K, Value_>> {
    ...
    static auto compose(detail::Acquire<K, Value_> acquire) {
        ...
        return detail::Acquire<decltype(k), Value>(std::move(k),
                                                   acquire.lock_);
    }
};
...
template<typename K, typename Value>
struct IsContinuation<detail::Release<K, Value>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Value>
struct HasTerminal<detail::Release<K, Value>> : HasTerminal<K> {};
...
struct Compose<detail::Release<K, Value_>> {
    ...
    static auto compose(detail::Release<K, Value_> release) {
        ...
        return detail::Release<decltype(k), Value>(std::move(k),
                                                   release.lock_);
    }
};
...
template<typename K, typename Context, typename Condition, typename Arg,
         typename Value>
struct IsContinuation<detail::Wait<K, Context, Condition, Arg, Value>> :
    std::true_type {};
...
template<typename K, typename Context, typename Condition, typename Arg,
         typename Value>
struct HasTerminal<detail::Wait<K, Context, Condition, Arg, Value>> :
    HasTerminal<K> {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Context, typename Condition, typename Arg_,
         typename Value>
struct Compose<detail::Wait<K, Context, Condition, Arg_, Value>> {
    template<typename Arg>
    static auto compose(
        detail::Wait<K, Context, Condition, Arg_, Value> wait) {
        return detail::Wait<K, Context, Condition, Arg, Value>(
            std::move(wait.k_), std::move(wait.context_),
            std::move(wait.condition_), wait.lock_);
    }
};
...
auto Wait(Lock* lock) {
    return detail::Wait<Undefined, Undefined, Undefined, Undefined, Value>(
        Undefined(), Undefined(), Undefined(), lock);
}
...
public:
    ...
    auto Synchronized(E e) const {
        return Acquire(lock_) | std::move(e) | Release(lock_);
    }
