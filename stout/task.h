...
struct Streamable<
    S, T, decltype(void(std::declval<S&>() << std::declval<T const&>()))> :
    std::true_type {};
...
struct FailedException : public std::exception {
    ...
    // constructor below.
    FailedException(const FailedException& that) : message_(that.message_) {}
...
template<typename E>
auto Terminate(E e) {
    using Value = std::conditional_t<IsUndefined<typename E::Value>::value,
                                     void, typename E::Value>;
            ...
            (Terminal()
                 ...
                 .fail([](auto& promise, auto&&... errors) {
                     promise.set_exception(
                         std::make_exception_ptr(FailedException(
...
struct TaskAdaptor {
    ...
    TaskAdaptor(Callback<Arg_>* start, Callback<std::exception_ptr>* fail,
                Callback<>* stop) :
        start_(start),
        fail_(fail),
        stop_(stop) {}
...
struct HeapTask {
    ...
    void Start(Interrupt& interrupt, Callback<typename E_::Value>&& start,
               Callback<std::exception_ptr>&& fail, Callback<>&& stop) {
    ...
    using Adaptor_ =
        typename EKPossiblyUndefined<E_,
...
struct TaskWith {
    ...
    template<typename F>
    TaskWith(Args_... args, F f) : args_(std::move(args)...) {
        static_assert(!IsContinuation<F>::value,
                      "'Task' expects a callable "
                      "NOT an eventual continuation");

        static_assert(sizeof...(args) > 0 || std::is_invocable_v<F>,
                      "'Task' expects a callable that "
                      "takes no arguments");

        static_assert(sizeof...(args) == 0 || std::is_invocable_v<F, Args_...>,
                      "'Task' expects a callable that "
        ...
        static_assert((HasTerminal<E>::value && terminated_) ||
        ...
        start_ = [f = std::move(f)](
                     ...
                     class Interrupt& interrupt, Callback<Value>&& start,
                     Callback<std::exception_ptr>&& fail, Callback<>&& stop) {
            if (!e_) {
            ...
            e->Start(interrupt, std::move(start), std::move(fail),
                     std::move(stop));
        };
    }
    ...
    void Start(Interrupt& interrupt, Callback<Value_>&& start,
               Callback<std::exception_ptr>&& fail, Callback<>&& stop) {
        std::apply(
            [&](auto&&... args) {
                start_(std::forward<decltype(args)>(args)..., e_, interrupt,
                       std::move(start), std::move(fail), std::move(stop));
            },
            std::move(args_));
    }
    ...
    Callback<Args_&&..., std::unique_ptr<void, Callback<void*>>&,
             class Interrupt&, Callback<Value_>&&,
             Callback<std::exception_ptr>&&, Callback<>&&>
        start_;
};
...
public:
    ...
    void Start(Interrupt& interrupt, Callback<Value_>&& start,
               Callback<std::exception_ptr>&& fail, Callback<>&& stop) {
        task_.Start(interrupt, std::move(start), std::move(fail),
                    std::move(stop));
    }
...
struct IsContinuation<detail::TaskAdaptor<Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename Arg>
struct HasTerminal<detail::TaskAdaptor<Arg>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Value, bool terminated, typename... Args>
struct IsContinuation<
    detail::TaskContinuation<K, Value, terminated, Args...>> :
    std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename K, typename Value, bool terminated, typename... Args>
struct HasTerminal<detail::TaskContinuation<K, Value, terminated, Args...>> :
    HasTerminal<K> {};

////////////////////////////////////////////////////////////////////////

template<typename Value, bool terminated>
struct IsContinuation<Task<Value, terminated>> : std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename Value, bool terminated>
struct HasTerminal<Task<Value, terminated>> :
    std::bool_constant<terminated> {};
...
template<typename Value, bool terminated, typename... Args>
struct IsContinuation<TaskWith<Value, terminated, Args...>> :
    std::true_type {};

////////////////////////////////////////////////////////////////////////

template<typename Value, bool terminated, typename... Args>
struct HasTerminal<TaskWith<Value, terminated, Args...>> :
    std::bool_constant<terminated> {};
